...
int main(int argc, char *argv[])
{
    ...
    if (!get_config("server1", server, sizeof(server)))
    { /*set default:router.eu.thethings.network*/
    ...
    *(uint32_t *)(buff_up + 8) = net_mac_l;
    ...
    rxlora(rxdev, RXMODE_SCAN); /* start lora continue receive mode */
    ...
    while (!exit_sig && !quit_sig)
    {
        ...
        { /* read IO if RXDONE */
            ...
            {
                //    MSG_LOG(DEBUG_INFO, "Someting is recevied:%s\n",&pktrx.size);
                push_mqtt(&pktrx);
            }
            pktrx_clean(&pktrx);
        }
...
static void push_mqtt(struct pkt_rx_s *rx_pkt)
{
    ...
    // Choose a safe size for tmp based on your maximum payload size
    #define MAX_TMP_SIZE 256

    size_t copy_size = (rx_pkt->size < MAX_TMP_SIZE) ? rx_pkt->size : MAX_TMP_SIZE - 1;
    strncpy(tmp, rx_pkt->payload, copy_size);
    tmp[copy_size] = '\0'; // Ensure null-termination

    // for (i = 0; i < rx_pkt->size; i++)
    //{
    //     tmp[i] = rx_pkt->payload[i];
    //     //   hex_data[i]=rx_pkt->payload[i];
    // }

    /*
        MSG_LOG(DEBUG_INFO, "Someting bit0: %x\n", &tmp[0]);
        MSG_LOG(DEBUG_INFO, "Someting bit1: %x\n",&tmp[1]);
        MSG_LOG(DEBUG_INFO, "Someting bit2: %x\n",&tmp[2]);
        MSG_LOG(DEBUG_INFO, "Someting bit3: %x\n",&tmp[3]);
        */
    //%02x
    /* for (i = 0; i < rx_pkt->size; i++) {
     ...
     printf("\n");*/
    // MSG_LOG(DEBUG_INFO, "INFO~  struct: data1=%d, data2=%d \n", data_size,hex_data);
    ...
    for (i = 0; i < 16; i++)
    { /* if radiohead lib then have 4 byte of RH_RF95_HEADER_LEN */
        if (tmp[i] == '<' && id_found == 0)
        { /* if id_found more than 1, '<' found  more than 1 */
            chan_id = &tmp[i + 1];
            ++id_found;
        }
    ...
    {
        clock_gettime(CLOCK_REALTIME, &fetch_time);
        x1 = gmtime(&(fetch_time.tv_sec));
        fprintf(fp, "{\"date\":\"%04i-%02i-%02iT%02i:%02i:%02i\",\"rssi\":\"%.0f\",\"snr\":\"%.0f\",\"payload\":\"", (x1->tm_year) + 1900, (x1->tm_mon) + 1, x1->tm_mday, x1->tm_hour, x1->tm_min, x1->tm_sec, rx_pkt->rssi, rx_pkt->snr); /* split the UNIX timestamp to its calendar components */
        MSG_LOG(DEBUG_INFO, "RSSI: %.0f, SNR: %.0f\n", rx_pkt->rssi, rx_pkt->snr);
        ...
        //  fprintf(fp,'"rssi":"');
        //  fprintf(fp, "%.0f,", rx_pkt->rssi);       snr

        for (jj = i_id; jj < rx_pkt->size; jj++)
        {
            fprintf(fp, "%02x", rx_pkt->payload[jj]);
        }
        fprintf(fp, "\"}");
        fflush(fp);
        fclose(fp);
